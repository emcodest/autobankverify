...
this.DeleteBatch = async (req, res) => {

    const {id} = req.body
    if(! id){
        return hl.Error(res, `ID is required`)
    }
    if(! req.user){
        return hl.Error(res, `You need to be logged in`)
    }
    //: check if user has permission to delete this batch
    let is_p = await hl.bindRawQuery("select id from batch_list where id = $idd and user_id = $uid", {idd: id, uid: req.user.id})
    if(is_p.length === 1){
        await hl.genDelete("batch_list", {id: id, user_id: req.user.id})
        return hl.Success(res, `Batch Deleted successfully`)
    }else{
        return hl.Error(res, `Unable to delete batch`)
    }
